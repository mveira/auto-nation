...
function parseHours(dayLabel: string, value?: string): OpeningHoursSpec | null {
...
export function buildAutoRepairJsonLd() {
  ...
  // In dev: allow placeholders so you can see schema working.
  // In prod: require the critical fields, otherwise return null (no JSON-LD output).
  ...
  const openingHoursSpecification: OpeningHoursSpec[] = [
    // Mon–Fri
    parseHours(["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"] as any, process.env.NEXT_PUBLIC_HOURS_MON_FRI) as any,
    // Sat
  ...
  // Only include aggregateRating if BOTH are present.
